...
static void sdhci_cv181x_sd_setup_pad(struct sdhci_host *host, bool bunplug)
{
	...
	if (cvi_host->has_cd_gpio)
		writeb(0x3, cvi_host->pinmuxbase + 0x34);
	else
		writeb(0x0, cvi_host->pinmuxbase + 0x34);

	if (of_property_read_bool(cvi_host->pdev->dev.of_node,
				  "cvitek,sd0-pwr-en-as-gpio"))
		writeb(0x3, cvi_host->pinmuxbase + 0x38);
	else
...
static unsigned int sdhci_cvi_general_get_max_clock(struct sdhci_host *host)
{
	...
	/*
	 * 5.10: mmc_of_parse("src-frequency") -> mmc->f_src, returned here.
	 * 5.15 mmc core has no f_src, so read the same DT property.
	 * max-frequency is the card cap, not the SDHCI input clock.
	 */
	if (device_property_read_u32(dev, "src-frequency", &src) || !src) {
		pr_err("%s: missing src-frequency in DT\n",
...
static void sdhci_cv181x_sd_dump_vendor_regs(struct sdhci_host *host)
{
	...
	SDHCI_DUMP(": Reg_244:   0x%08x | Reg_248:  0x%08x\n",
		   sdhci_readl(host, CVI_CV181X_SDHCI_PHY_DS_DLY),
		   sdhci_readw(host, CVI_CV181X_SDHCI_PHY_DLY_STS));
	SDHCI_DUMP(": Reg_24C:   0x%08x | has_cd:  0x%08x\n",
		   sdhci_readl(host, CVI_CV181X_SDHCI_PHY_CONFIG),
		   cvi_host->has_cd_gpio);
...
static int sdhci_cvi_probe(struct platform_device *pdev)
{
	...
	pr_info("cvi: %s flags=0x%x adma=%d sdma=%d dma64=%d max_clk=%u f_max=%u\n",
		mmc_hostname(host->mmc), host->flags,
		!!(host->flags & SDHCI_USE_ADMA),
		!!(host->flags & SDHCI_USE_SDMA),
		!!(host->flags & SDHCI_USE_64_BIT_DMA),
		host->max_clk, host->mmc->f_max);
