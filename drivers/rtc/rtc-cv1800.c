...
/*
 * RTC driver for Sophgo CV1800 series SoCs
 *
...
/* These registers remain powered in the VDDBKUP domain. */
...
static void cv1800_rtc_enable(struct device *dev)
{
	...
	/* Generate the one-second pulse internally. */
	regmap_update_bits(info->rtc_map, SEC_PULSE_GEN, SEL_SEC_PULSE, 0);
}

static int cv1800_rtc_alarm_irq_enable(struct device *dev,
